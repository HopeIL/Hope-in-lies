...
void interactive_day_1()
{
    ...
    while(dey != 4)
    {
        ...
        if(dey == 3)
        {
            ...
            wait(1000);
            string sym = "qwertyuiop[]asdfghjkl;'zxcvbnm`1234567890-=*-+QWERTYUIOP{}ASDFGHJKLL:ZXCVBNM<>,./";
            for(int i = 0; i < 29998 * 4; i++)
            {
                cout <<sym[gen() % sym.size()];
            }
            clr;
        ...
        if(dey == 6 && act[5] == 0){
            print("6.ДИМА! тут какое-нибудь действие типа просто постоять у окна(его нет) или типа того.", 60);
        }
        int root = 0;
        for(int i = 0; i < 4; i++)
            root += act[i];
...
void interactive_day_1_2(){
    do{
        print("1.Подойти к телевизору.\n", 0);
        print("2.Оглядеть книгу.\n", 0);
        print("3.Осмотреть монитор.\n", 0);
        print("4.Лечь спать.\n", 0);
        string s;
        cin >> s;
        int act [] = {1, 1, 1, 1};
